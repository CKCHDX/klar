...
class ServerThread(QThread):
    ...
    def __init__(self, host: str = "localhost", port: int = 5000, public_url: Optional[str] = None):
        """Initialize server thread"""
        super().__init__()
        self.host = host
        self.port = port
        self.server_url = public_url or f"http://{host}:{port}"
        self.server_process: Optional[Process] = None
        self.is_running = False
        
        # Log configuration
        logger.info(f"ServerThread initialized with host={host}, port={port}, public_url={public_url}")
        logger.info(f"Server will be accessible at: {self.server_url}")
    ...
    def run(self):
        ...
        try:
            ...
            # Display public URL if available, otherwise show host:port
            display_url = self.server_url if self.server_url else f"http://{self.host}:{self.port}"
            self.status_changed.emit(f"Server initializing on {display_url}")
...
class Phase3ServerBootstrap(QWizardPage):
    ...
    def _start_server(self):
        ...
        try:
            ...
            self.server_thread = ServerThread(host=host, port=port, public_url=public_url)
