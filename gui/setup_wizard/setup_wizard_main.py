...
class SetupWizard(QWizard):
    ...
    def _save_completion_status(self):
        ...
        try:
            ...
            logger.info("Setup completion status saved to config")
            
            # Also mark in StateManager for server-side tracking
            try:
                ...
                logger.info("Setup marked complete in StateManager")
            except Exception as e:
                logger.warning(f"Failed to update StateManager: {e}")
                # Non-critical, config is the primary source
