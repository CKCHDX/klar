    ...
    def run(self):
        ...
        try:
            ...
            for idx, domain_info in enumerate(domains):
                ...
                try:
                    # Crawl domain
                    stats = self.crawler.crawl_domain(domain, start_urls=domain_info.get('seed_urls'))

                    # If robots.txt blocks everything, retry once without robots to hit target pages
                    if (
                        stats.get('pages_crawled', 0) == 0 and
                        self.config.get('respect_robots', True) and
                        self.config.get('force_ignore_robots_on_zero', True)
                    ):
                        self.log_message.emit(
                            'warning',
                            f"⚠ {domain} blocked by robots.txt. Retrying without robots.txt to reach target pages..."
                        )
                        stats = self.crawler.crawl_domain(
                            domain,
                            start_urls=domain_info.get('seed_urls'),
                            force_ignore_robots=True
                        )
